...
library('foreign') # 외부 데이터 파일을 불러오기 위한 패키지.
library('ggplot2')
...
{
  no.pres <- subset(df, state < 99) # 부통령 제외, 부통령은 보통 투표를 하지 않기 때문.
  
  for(i in 10:ncol(no.pres))
  {
    no.pres[,i] <- ifelse(no.pres[,i] > 6, 0, no.pres[,i]) # 큰 수부터 0으로 치환.
    no.pres[,i] <- ifelse(no.pres[,i] > 0 & no.pres[,i] < 4, 1, no.pres[,i]) # 기타 조건 1로 치환.
    no.pres[,i] <- ifelse(no.pres[,i] > 1, -1, no.pres[,i]) # 기타 모두 -1로 치환.
  }
  
  return(as.matrix(no.pres[,10:ncol(no.pres)]))
}

rollcall.simple <- lapply(rollcall.data, rollcall.simplified) # lapply - 리스트 반환.
View(rollcall.simple)
...
rollcall.dist <- lapply(rollcall.simple, function(m) dist(m %*% t(m))) # 따로 정방행렬을 만들지 않고, 바로 거리 구하기.
View(rollcall.dist)
# Do the multidimensional scaling
rollcall.mds <- lapply(rollcall.dist,
                       function(d) as.data.frame((cmdscale(d, k = 2)) * -1)) # cmdscale로 좌표 추출.
...
### test
congress <- subset(rollcall.data[[1]], state < 99)
head(congress)
test <- as.character(congress$name)
strsplit(test[1], "[, ]")[[1]][1]
rm(congress)
###
...
for(i in 1:length(rollcall.mds))
{
  ...
  congress.names <- sapply(as.character(congress$name),
                           function(n) strsplit(n, "[, ]")[[1]][1]) # "SHELBY, RIC" "HEFLIN, HOW" 이런 이름에서 First name만 가져옴.
...
all.plot <- ggplot(all.mds, aes(x = x, y = y)) +
  ...
  ggtitle("Roll Call Vote MDS Clustering for U.S. Senate (101st - 111th Congress)") +
       ...
       scale_shape(name = "Party",
                   ...
                   labels = c("Dem.", "Rep.", "Ind."),
                   solid = FALSE) +
      facet_wrap(~ congress) # congress별로 묶음 시각화.
